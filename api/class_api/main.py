...
async def create_class(name: str, description: str):
    result = create_classrooom_db(name=name, description=description)
    return {'status': 1, 'result': result}


@class_router.post('/get_class')
...
async def update_class(cid: int, name: str, description: str):
    result = update_class_db(cid=cid, name=name, description=description)
    return {'status': 1, 'message': result}


@class_router.post('/delete_class')
