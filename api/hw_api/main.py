...
async def create_homework_api(name: str, description: str, group_id: int, uid: int, points: int):
    result = create_hw_db(name=name, description=description, group_id=group_id, uid=uid, points=points)
    return {'status': 1, 'message': result}

@hw_router.post('/get_homework')
async def get_hw(hid: int):
    result = get_all_or_exact_hw_db(hid=hid)
    return {'status': 1, 'message': result}

@hw_router.post('/delete_homework')
async def delete_hw(hid: int):
    result = delete_hw_db(hid=hid)
    return {'status': 1, 'message': result}

@hw_router.post('/update_homework')
